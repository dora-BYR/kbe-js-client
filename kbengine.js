...
/*-----------------------------------------------------------------------------------------
												debug
-----------------------------------------------------------------------------------------*/
KBEngine.INFO_MSG = function(...args)
{
	console.info("[KBE]", ...args);
}

KBEngine.DEBUG_MSG = function(...args)
{
	console.debug("[KBE]", ...args);
}

KBEngine.ERROR_MSG = function(...args)
{
	console.error("[KBE]", ...args);
}

KBEngine.WARNING_MSG = function(...args)
{
	console.warn("[KBE]", ...args);
}
...
KBEngine.Event = function()
{
	...
	{
		var callbackfn = typeof strCallback == "function" ? strCallback : classinst[strCallback];
		if(callbackfn == undefined)
		{
			KBEngine.ERROR_MSG('KBEngine.Event::register: not found strCallback(' + classinst  + ")!"+strCallback);  
			return;
		}
...
KBEngine.MemoryStream = function(size_or_buffer)
{
	...
	this.readBlob = function()
	{
		...
		// fixed by buptlong@126.com
		// if (size > this.buffer.byteLength - this.rpos) {
		// 	size = this.buffer.byteLength - this.rpos;
		// }
		// if (size < 0) {
		// 	size = this.buffer.byteLength - this.rpos;
		// }
...
KBEngine.Entity = KBEngine.Class.extend(
{
	...
	baseCall : function()
	{
		...
		// KBEngine.INFO_MSG("接口参数对比-basecall", arguments, args);
		...
		this.base.newCall();
		this.base.bundle.writeUint16(0); //适配组件 by buptlong@126.com
		this.base.bundle.writeUint16(methodID);

		KBEngine.DEBUG_MSG("baseCall: ", arguments, args);
	...
	cellCall: function()
	{
		...
		var methodID = method[0];
		var args = method[3];
		KBEngine.INFO_MSG("接口参数对比 - cellcall", arguments, args);
		if(arguments.length - 1 != args.length)
		{
		...
		this.cell.newCall();
		this.cell.bundle.writeUint16(0); //适配组件 by buptlong@126.com
...
KBEngine.DATATYPE_PYTHON = function()
{
	...
	{
		// return false;
		return true; // by longhui 20200628
	}
}
...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	this.reset = function()
	{
		...
		KBEngine.app.clientVersion = "1.3.12";
	...
	*/
	...
	{  
		...
		{
			var bundle = KBEngine.Bundle.createObject();
			KBEngine.INFO_MSG("KBEngine消息定义:", KBEngine.messages);
	...
	this.Client_onLoginSuccessfully = function(args)
	{
		...
		KBEngine.app.baseappPort = (this.useUdp && KBEngine.app.baseappUdpPort) ? KBEngine.app.baseappUdpPort : KBEngine.app.baseappTcpPort;
		KBEngine.app.serverdatas = args.readBlob();
	...
	this.onRemoteMethodCall_ = function(eid, stream)
	{
		...
		var propertyUtype = 0;
		var methodUtype = 0;
		if (KBEngine.moduledefs[entity.className].usePropertyDescrAlias) {
		...
		// KBEngine.INFO_MSG("KBEngine.moduledefs", entity.className, propertyUtype, methodUtype, KBEngine.moduledefs[entity.className].methods, KBEngine.moduledefs[entity.className].propertys);
		if (propertyUtype == 0) {
			// 实体方法
	...
	this.Client_onEntityEnterSpace = function(stream)
	{
		...
		KBEngine.app.entityServerPos.y = entity.position.y;
		KBEngine.app.entityServerPos.z = entity.position.z;
		entity.enterSpace(KBEngine.app.spaceID);
	}
